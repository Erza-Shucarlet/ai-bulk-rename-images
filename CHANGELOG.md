...
### Fixed
- 预览表格「状态」列固定在右侧（sticky），横向滚动时不随内容移动
...
### Added
- 预览表格支持鼠标拖拽横向滚动，文件名不再截断，可拖动查看完整名称
...
### Added
- 查找替换规则新增「包含扩展名」勾选项，勾选后可对文件名（含扩展名）整体进行查找替换
- 预览表格新增 checkbox，支持选择部分文件进行改名，全选/取消全选头部控件；未选中文件将被跳过
- 新增「撤销」按钮，执行改名后可一键恢复上一次操作的所有文件名
...
### Fixed
- index.html 和 privacy.html 添加 canonical 标签，声明各自权威 URL
- _redirects 新增 /privacy.html → /privacy 的 301 重定向，消除重复页面
